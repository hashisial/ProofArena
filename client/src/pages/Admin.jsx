...
function AdminStatCard({ detail, label, value }) {
  return (
    <Card variant="default">
      ...
      <p className="mt-3 text-4xl font-black tracking-[-0.06em] text-[#07030D]">{value}</p>
      <p className="mt-2 text-sm leading-6 text-[#6F657C]">{detail}</p>
    </Card>
  );
}
...
export function Admin() {
  return (
    <div className="grid gap-8">
      <section className="relative overflow-hidden rounded-[2rem] border border-[#7C3AED]/16 bg-[radial-gradient(circle_at_92%_12%,rgba(124, 58, 237, 0.14),transparent_32%),linear-gradient(135deg,#ffffff,#F8F4FF)] p-6 shadow-[0_24px_80px_rgba(31, 14, 54, 0.08)] md:p-8">
        <div className="relative grid gap-8 xl:grid-cols-[minmax(0,1fr)_22rem] xl:items-end">
          <div>
            <Badge variant="black">Platform admin</Badge>
            <h2 className="mt-5 max-w-4xl break-words text-3xl font-black tracking-[-0.055em] text-[#07030D] sm:text-4xl md:text-6xl md:tracking-[-0.065em]">
          ...
          <Card className="bg-white/90" variant="elevated">
            ...
            <CardContent className="grid gap-3">
              {trustItems.map((item) => (
                <div
                  className="flex flex-col items-start gap-3 rounded-2xl border border-[#E9E2F3] bg-[#FAFAFA] px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4"
                  key={item}
                >
                  <span className="min-w-0 break-words text-sm font-black text-[#07030D]">{item}</span>
      ...
      <section aria-labelledby="admin-stats-title">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 id="admin-stats-title" className="mt-2 text-2xl font-black tracking-[-0.04em] text-[#07030D]">
      ...
      <section aria-labelledby="admin-actions-title">
        <div>
          ...
          <h2 id="admin-actions-title" className="mt-2 text-2xl font-black tracking-[-0.04em] text-[#07030D]">
        ...
        <Card variant="muted">
          ...
          <CardContent className="grid gap-3">
            {[
              ...
              <div className="rounded-2xl border border-[#E9E2F3] bg-white px-4 py-3" key={title}>
                <div className="flex items-center gap-3">
                  <Users aria-hidden="true" className="h-4 w-4 text-[#7C3AED]" />
