...
export function PlanComparisonCard({ plan }) {
  return (
    <Card padding="md" variant="muted">
      ...
      <h3 className="mt-3 text-lg font-black text-[#1C1917]">{plan?.title}</h3>
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#78716C]">Timeline</p>
          <p className="mt-1 font-bold text-[#1C1917]">{formatPlanTimeline(plan?.timeline)}</p>
        </div>
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#78716C]">Price</p>
          <p className="mt-1 font-bold text-[#1C1917]">{formatPlanPrice(plan?.price)}</p>
        </div>
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#78716C]">Proof</p>
          <p className="mt-1 font-bold text-[#1C1917]">{plan?.proofPlan?.length ?? 0} items</p>
        </div>
      </div>
    </Card>
  );
