...
export function ExecutionPlanQualityCard({ plan }) {
  ...
  return (
    <Card padding="md" variant="default">
      <p className="text-xs font-black uppercase tracking-[0.16em] text-[#3F6212]">Plan quality</p>
      <div className="mt-3 flex items-end justify-between gap-4">
        <div>
          <p className="text-4xl font-black tracking-normal text-[#1C1917]">{score}/100</p>
          <p className="mt-1 text-sm font-bold text-[#78716C]">{label}</p>
        </div>
      </div>
      <div
        aria-label={`Execution plan quality score ${score} out of 100`}
        className="mt-4 h-3 overflow-hidden rounded-full bg-[#E7E5E4]"
        role="progressbar"
        aria-valuemin={0}
        aria-valuemax={100}
        aria-valuenow={score}
      >
        <div className="h-full rounded-full bg-[#3F6212]" style={{ width: `${Math.min(score, 100)}%` }} />
      </div>
      ...
      {strengths.length > 0 ? (
        <div className="mt-4 grid gap-2">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#78716C]">Strengths</p>
          {strengths.slice(0, 4).map((item) => (
            <span className="flex items-center gap-2 text-sm font-semibold text-[#365314]" key={item}>
      ...
      {missingFields.length > 0 ? (
        <div className="mt-4 grid gap-2">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#78716C]">Improve next</p>
          {missingFields.slice(0, 5).map((item) => (
            <span className="flex items-center gap-2 text-sm font-semibold text-[#78716C]" key={item}>
