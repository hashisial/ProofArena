...
export function ProviderPlanPerformancePanel({ plan }) {
  ...
  return (
    <Card as="section" aria-labelledby="plan-performance-title" padding="lg" variant="bordered">
      <p className="text-xs font-black uppercase tracking-[0.16em] text-[#3F6212]">Performance</p>
      <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-[#1C1917]" id="plan-performance-title">Plan performance</h2>
          <p className="mt-2 text-sm leading-6 text-[#78716C]">
            {hasScore ? getExecutionPlanQualityLabel(Number(score)) : "Plan score will appear when available."}
          </p>
        </div>
        <p className="text-4xl font-black text-[#1C1917]">{hasScore ? `${score}/100` : "Not available"}</p>
      </div>
      {hasScore ? (
        <div
          ...
          role="progressbar"
        >
          <div className="h-full rounded-full bg-[#3F6212]" style={{ width: `${Math.min(Number(score), 100)}%` }} />
        </div>
      ) : null}
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        <div>
          <p className="text-sm font-black text-[#1C1917]">Strengths</p>
          <div className="mt-3 grid gap-2">
            {strengths.length > 0 ? strengths.map((item) => (
              <p className="flex items-start gap-2 text-sm leading-6 text-[#365314]" key={item}>
                <CheckCircle2 aria-hidden="true" className="mt-1 h-4 w-4 shrink-0" />
                {item}
              </p>
            )) : <p className="text-sm leading-6 text-[#78716C]">Strengths will appear as the plan becomes more complete.</p>}
          </div>
        </div>
        <div>
          <p className="text-sm font-black text-[#1C1917]">Missing fields and warnings</p>
          <div className="mt-3 grid gap-2">
            {[...missingFields, ...warnings].length > 0 ? [...missingFields, ...warnings].map((item, index) => (
              <p className="flex items-start gap-2 text-sm leading-6 text-[#57534E]" key={`${item}-${index}`}>
                <CircleAlert aria-hidden="true" className="mt-1 h-4 w-4 shrink-0 text-[#A16207]" />
                {String(item).replaceAll("_", " ")}
              </p>
            )) : <p className="text-sm leading-6 text-[#78716C]">No missing fields or warnings are currently recorded.</p>}
          </div>
        </div>
      </div>
      {feedback ? (
