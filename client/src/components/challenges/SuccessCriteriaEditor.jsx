...
export function SuccessCriteriaEditor({ error, items = [], onChange }) {
  ...
  return (
    <section className="grid gap-4" aria-labelledby="challenge-success-criteria-title">
      <div>
        <h3 className="text-lg font-black text-[#1C1917]" id="challenge-success-criteria-title">
          Success criteria
        </h3>
        <p className="mt-1 text-sm leading-6 text-[#78716C]">
          Define what providers must deliver before the challenge is accepted.
        </p>
      </div>
        ...
        {criteria.map((item, index) => (
          <div className="grid gap-4 rounded-2xl border border-[#E7E5E4] bg-[#FEFCE8] p-4" key={index}>
            <div className="flex items-start justify-between gap-3">
              <p className="text-sm font-black text-[#3F6212]">Criterion {index + 1}</p>
              <button
            ...
            <Input
              ...
              placeholder="Each lead includes name, phone, email, source, status, and duplicate check."
              value={item.title}
            />
            <Textarea
              label="Description"
              onChange={(event) => updateItem(index, { description: event.target.value })}
              placeholder="Explain the acceptance rule in plain language."
              rows={3}
