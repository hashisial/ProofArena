...
export function AdminRecentList({ description, items = [], title }) {
  return (
    <Card className="h-full" variant="bordered">
      ...
      <div className="mt-5 divide-y divide-[#E9E2F3]">
        {items.length ? items.map((item) => (
          <div className="flex flex-col gap-2 py-3 first:pt-0 sm:flex-row sm:items-center sm:justify-between" key={item.id}>
            <div className="min-w-0">
              <p className="truncate text-sm font-black text-[#07030D]">{getTitle(item)}</p>
              <p className="mt-1 text-xs font-bold text-[#6F657C]">{formatDate(item.createdAt)}</p>
            </div>
            <AdminStatusBadge status={getStatus(item)} />
          </div>
        )) : (
          <p className="text-sm leading-6 text-[#6F657C]">No recent records available.</p>
        )}
      </div>
    </Card>
  );
