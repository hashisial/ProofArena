  ...
  if (matrix.length === 0) {
    return (
      <Card className="rounded-3xl" padding="lg" variant="muted">
        <p className="text-sm font-black text-[#07030D]">Comparison highlights unavailable</p>
    ...
    <Card className="rounded-3xl" padding="lg">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {matrix.map((item) => {
          ...
          return (
            <div
              className="rounded-2xl border border-[#DDD6FE] bg-[#F5F3FF] p-4"
              key={item.key}
            >
              <Badge variant="green">{item.label}</Badge>
              <p className="mt-3 text-sm font-black leading-6 text-[#07030D]">
                {winnerNames.join(", ")}
              </p>
