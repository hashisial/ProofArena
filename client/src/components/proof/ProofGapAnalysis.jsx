      ...
      <CardContent className="grid gap-5">
        {actions.length > 0 ? (
          <ol className="grid gap-3">
            {actions.map((action) => (
              <li className="rounded-2xl border border-[#E9E2F3] bg-[#F8F4FF] p-4" key={action.key}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <CircleAlert aria-hidden="true" className="h-4 w-4 shrink-0 text-[#6D28D9]" />
                      <p className="font-black text-[#07030D]">{action.title}</p>
                    </div>
        ...
        <div>
          <h3 className="text-sm font-black text-[#07030D]">Suggested places to use proof</h3>
          {suggestions.length > 0 ? (
            <ul className="mt-3 grid gap-3">
              {suggestions.slice(0, 4).map((suggestion) => (
                <li className="flex flex-col gap-3 rounded-2xl border border-[#E9E2F3] bg-white p-4 sm:flex-row sm:items-center sm:justify-between" key={`${suggestion.assetId}-${suggestion.contextType}-${suggestion.contextTitle}`}>
                  <div className="min-w-0">
                    <p className="flex items-center gap-2 text-sm font-black text-[#07030D]">
                      <Link2 aria-hidden="true" className="h-4 w-4 shrink-0 text-[#7C3AED]" />
