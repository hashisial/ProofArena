...
export function ProofAssetCard({
  ...
  return (
    <Card as="article" className="flex h-full flex-col" variant="bordered">
      ...
      <h2 className="mt-4 text-xl font-black tracking-normal text-[#1C1917]">{asset?.title || "Untitled proof asset"}</h2>
      <p className="mt-2 line-clamp-3 min-h-[4.5rem] text-sm leading-6 text-[#78716C]">
      ...
      <div className="mt-3 rounded-2xl border border-[#E7E5E4] bg-white p-3">
        <div className="flex items-center justify-between gap-3">
          <p className="text-xs font-black uppercase tracking-[0.12em] text-[#78716C]">Quality completeness</p>
          <p className="text-sm font-black text-[#1C1917]">{quality.score}%</p>
        </div>
        <div aria-label={`Proof asset quality ${quality.score} percent`} className="mt-2 h-2 overflow-hidden rounded-full bg-[#F5F5F4]" role="progressbar" aria-valuemax={100} aria-valuemin={0} aria-valuenow={quality.score}>
          <div className="h-full rounded-full bg-[#3F6212]" style={{ width: `${quality.score}%` }} />
        </div>
      </div>
      {tags.length > 0 ? (
