...
function ProductPreview() {
  return (
    <Card
      className="premium-tilt premium-motion-card relative overflow-hidden rounded-[2rem] border-[#EDE9FE] shadow-[0_30px_90px_rgba(124, 58, 237, 0.16)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_34px_100px_rgba(124, 58, 237, 0.2)]"
      padding="none"
      variant="elevated"
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_12%,rgba(124, 58, 237, 0.14),transparent_30%),linear-gradient(135deg,#FFFFFF_0%,#F8F4FF_52%,#F5F3FF_100%)]" />
      <div className="absolute inset-y-6 left-0 w-1 rounded-r-full bg-[#7C3AED]" />
      <div className="relative z-10 p-4 sm:p-5">
        <div className="rounded-[1.5rem] border border-[#EDE9FE] bg-white p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-5 text-xs font-black uppercase tracking-[0.14em] text-[#7C3AED]">
                Outcome Challenge
              </p>
              <h2 className="mt-2 max-w-xl break-words text-2xl font-black leading-tight text-[#07030D] sm:text-3xl">
          ...
          <div className="mt-6 rounded-3xl border border-[#EDE9FE] bg-[#F8F4FF] p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm font-bold text-[#493C5E]">Plan selected, milestones moving, proof pending.</p>
              </div>
              <Badge variant="green">3/5 completed</Badge>
            </div>
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-[#EDE9FE]">
              <div className="h-full w-[60%] rounded-full bg-[#7C3AED]" />
            </div>
          </div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {challengeDetails.map((metric) => {
              ...
              return (
                <div
                  className="flex min-w-0 items-start gap-3 rounded-2xl border border-[#E9E2F3] bg-white p-4"
                  key={metric.label}
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 break-words text-lg font-black text-[#07030D]">
                      {metric.value}
                    </p>
                  </div>
                </div>
            ...
            <div className="mt-4 grid gap-3">
              {statusRows.map((item, index) => (
                <div className="flex items-center gap-3 rounded-2xl bg-white p-3" key={item.label}>
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-[#F5F3FF] text-xs font-black text-[#5B21B6]">
                    {index + 1}
                  </span>
                  <span className="min-w-0 text-sm font-black text-[#07030D]">{item.label}</span>
                  {item.complete ? (
                    <CheckCircle2 aria-hidden="true" className="ml-auto h-4 w-4 shrink-0 text-[#A78BFA]" />
                  ) : (
                    <BadgeCheck aria-hidden="true" className="ml-auto h-4 w-4 shrink-0 text-[#7C3AED]" />
                  )}
                </div>
              ))}
            </div>
...
export function MarketplaceHeroSection() {
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-[#E9E2F3] bg-[#F8F4FF]">
      <div className="absolute inset-0 -z-10 bg-[linear-gradient(135deg,#FFFFFF_0%,#F8F4FF_46%,#F5F3FF_100%)]" />
      <div className="absolute inset-x-0 top-0 -z-10 h-[36rem] bg-[radial-gradient(circle_at_18%_12%,rgba(124, 58, 237, 0.18),transparent_34%),radial-gradient(circle_at_92%_24%,rgba(167, 139, 250, 0.18),transparent_28%)]" />
      <Container className="grid gap-10 py-14 sm:py-20 lg:min-h-[720px] lg:grid-cols-[minmax(0,0.98fr)_minmax(0,1.02fr)] lg:items-center lg:gap-14 lg:py-24">
        <div className="min-w-0">
          ...
          <MotionDiv {...getMotionProps(reduceMotion, 0.06)}>
            <h1 className="mt-6 max-w-5xl break-words text-[2.35rem] font-black leading-[1.03] text-[#07030D] [text-wrap:balance] min-[390px]:text-5xl sm:text-6xl lg:text-7xl">
          ...
          >
            ...
            <a
              className="inline-flex min-h-12 w-full max-w-full items-center justify-center rounded-full px-4 text-center text-sm font-black text-[#5B21B6] transition hover:bg-white/80 hover:text-[#7C3AED] focus:outline-none focus:ring-2 focus:ring-[#A78BFA]/70 sm:w-auto"
          ...
          <MotionDiv {...getMotionProps(reduceMotion, 0.24)}>
            <div className="mt-7 flex max-w-2xl items-start gap-3 rounded-3xl border border-[#E9E2F3] bg-white/82 p-4 shadow-[0_18px_54px_rgba(31, 14, 54, 0.06)] backdrop-blur">
              <ShieldCheck aria-hidden="true" className="mt-1 h-5 w-5 shrink-0 text-[#7C3AED]" />
              <p className="text-sm font-bold leading-7 text-[#6F657C]">
