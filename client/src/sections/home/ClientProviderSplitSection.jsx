...
function PreviewCard({ panel, provider = false }) {
  ...
  return (
    <div className="mt-7 rounded-[1.5rem] border border-[#E9E2F3] bg-white p-4 shadow-[0_16px_48px_rgba(31, 14, 54, 0.05)] sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-base font-black text-[#07030D]">{panel.previewTitle}</p>
            <p className="mt-1 break-words text-sm font-bold leading-6 text-[#6F657C]">{panel.example}</p>
          </div>
        </div>
      ...
      {provider ? (
        <div className="mt-5 rounded-2xl border border-[#EDE9FE] bg-[#F8F4FF] p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="text-lg font-black text-[#07030D]">92</span>
          </div>
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#EDE9FE]">
      ...
      <div className="mt-4 grid gap-3">
        {panel.previewRows.map((row, index) => (
          <div className="flex items-center gap-3 rounded-2xl border border-[#E9E2F3] bg-[#F8F4FF] p-3" key={row}>
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-white text-xs font-black text-[#5B21B6]">
              {index + 1}
            </span>
            <span className="min-w-0 text-sm font-black leading-6 text-[#07030D]">{row}</span>
            {index >= 2 ? (
              <CheckCircle2 aria-hidden="true" className="ml-auto h-4 w-4 shrink-0 text-[#7C3AED]" />
            ) : null}
          </div>
        ))}
      </div>
...
export function ClientProviderSplitSection() {
  ...
  return (
    <section className="relative overflow-hidden bg-[#F8F4FF] py-16 sm:py-20 lg:py-24">
      <div className="absolute inset-x-0 top-0 h-[32rem] bg-[radial-gradient(circle_at_18%_12%,rgba(124, 58, 237, 0.13),transparent_34%),radial-gradient(circle_at_88%_74%,rgba(167, 139, 250, 0.12),transparent_28%)]" />
      <Container className="relative z-10">
        <MotionDiv {...getMotionProps(reduceMotion, 0)}>
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mx-auto mt-5 max-w-3xl text-base leading-8 text-[#6F657C] sm:text-lg">
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-2">
          {panels.map((panel, index) => {
            ...
            return (
              ...
              >
                <Card
                  className={`premium-motion-card group h-full border-[#E9E2F3] transition duration-300 hover:-translate-y-1 hover:border-[#A78BFA] hover:shadow-[0_28px_86px_rgba(124, 58, 237, 0.16)] ${
                    isProvider
                      ? "bg-[linear-gradient(135deg,#FFFFFF_0%,#F8F4FF_54%,#F5F3FF_100%)]"
                      : "bg-white"
                  }`}
                  padding="lg"
                  variant="elevated"
                >
                  ...
                  <h2 className="mt-6 break-words text-3xl font-black leading-tight text-[#07030D] [text-wrap:balance] sm:text-4xl">
                    {panel.title}
                  </h2>
                  <p className="mt-4 text-sm font-semibold leading-7 text-[#6F657C] sm:text-base">
                    {panel.description}
                  </p>
                  ...
                  <ul className="mt-6 grid gap-3">
                    {panel.bullets.map((bullet) => (
                      <li className="flex items-start gap-3" key={bullet}>
                        ...
                        <span className="text-sm font-semibold leading-7 text-[#493C5E]">{bullet}</span>
                      </li>
                    ))}
                  </ul>
                  ...
                  <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                    ...
                    <a
                      className="inline-flex min-h-12 w-full max-w-full items-center justify-center rounded-full border border-[#E9E2F3] bg-white px-4 text-center text-sm font-black text-[#5B21B6] transition hover:-translate-y-0.5 hover:border-[#A78BFA] hover:bg-[#F5F3FF] focus:outline-none focus:ring-2 focus:ring-[#A78BFA]/70 sm:w-auto"
