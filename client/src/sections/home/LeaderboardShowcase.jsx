...
function StatBlock({ label, value }) {
  return (
    <div className="min-w-0 rounded-2xl border border-[#E9E2F3] bg-white px-3 py-3">
      <p className="text-[0.68rem] font-black uppercase tracking-[0.12em] text-[#6F657C]">
        {label}
      </p>
      <p className="mt-1 text-base font-black leading-6 text-[#07030D]">{value}</p>
    </div>
  );
}
...
function LeaderboardRow({ index, reduceMotion, row }) {
  ...
  return (
    <MotionArticle
      ...
      className="premium-motion-row group min-w-0 rounded-3xl border border-[#E9E2F3] bg-white p-4 shadow-[0_14px_42px_rgba(31,14,54,0.05)] transition duration-300 hover:-translate-y-1 hover:border-[#A78BFA] hover:bg-[#F8F4FF] hover:shadow-[0_24px_70px_rgba(124,58,237,0.13)]"
    >
      <div className="grid min-w-0 gap-4 lg:grid-cols-[72px_minmax(0,1fr)_104px_112px_92px_92px_minmax(112px,auto)] lg:items-center">
        <div className="flex min-w-0 items-center gap-3 lg:block">
          <span
            className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl text-sm font-black shadow-[0_12px_26px_rgba(124,58,237,0.12)] ${
              index === 0
                ? "bg-[#7C3AED] text-white"
                : "border border-[#EDE9FE] bg-[#F5F3FF] text-[#5B21B6]"
            }`}
          >
            {rank}
          </span>
          <div className="min-w-0 lg:hidden">
            <h3 className="break-words text-base font-black leading-6 text-[#07030D]">
              {row.provider}
            </h3>
            <p className="mt-1 break-words text-sm font-semibold leading-6 text-[#6F657C]">
              {row.specialty}
            </p>
          </div>
        </div>
        ...
        <div className="hidden min-w-0 lg:block">
          <h3 className="truncate text-base font-black leading-6 text-[#07030D]">
            {row.provider}
          </h3>
          <p className="mt-1 truncate text-sm font-semibold leading-6 text-[#6F657C]">
            {row.specialty}
          </p>
        </div>

        <div className="rounded-2xl border border-[#A78BFA]/20 bg-[#F5F3FF] px-3 py-3">
          <div className="flex items-center justify-between gap-3 lg:block">
            <p className="text-[0.68rem] font-black uppercase tracking-[0.12em] text-[#7C3AED]">
              Proof Score
            </p>
            <p className="text-xl font-black leading-7 text-[#7C3AED] lg:mt-1">
              {row.proofScore}
            </p>
          </div>
...
function ReputationPanel({ reduceMotion }) {
  return (
    <MotionDiv {...getMotionProps(reduceMotion, 0.16)}>
      <Card
        className="premium-motion-card h-full rounded-[2rem] border-[#EDE9FE] bg-white shadow-[0_24px_80px_rgba(124,58,237,0.1)]"
        padding="lg"
        variant="elevated"
      >
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  <h4 className="text-sm font-black leading-6 text-[#07030D]">
                    {item.title}
                  </h4>
...
function ProofScorePanel({ reduceMotion }) {
  return (
    <MotionDiv {...getMotionProps(reduceMotion, 0.22)}>
      <Card
        className="premium-motion-card h-full rounded-[2rem] bg-[linear-gradient(135deg,#FFFFFF_0%,#F8F4FF_58%,#F5F3FF_100%)]"
        padding="lg"
        variant="elevated"
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#EDE9FE] text-[#7C3AED]">
            <Gauge aria-hidden="true" className="h-6 w-6" />
          </span>
        </div>

        <div className="mt-7 rounded-[2rem] border border-[#7C3AED]/20 bg-white p-5">
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.14em] text-[#6F657C]">
                Example score
              </p>
              <p className="mt-2 text-5xl font-black leading-none text-[#7C3AED]">
                96
                <span className="text-xl text-[#6F657C]">/100</span>
              </p>
            </div>
            <Medal aria-hidden="true" className="h-10 w-10 text-[#6D28D9]" />
          </div>
          <div className="mt-5 h-3 overflow-hidden rounded-full bg-[#EDE9FE]">
            <div className="motion-score-fill h-full w-[96%] rounded-full bg-[#7C3AED]" />
          </div>
        </div>
...
export function LeaderboardShowcase() {
  ...
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(180deg,#FFFFFF_0%,#FBF9FF_45%,#F8F4FF_100%)] py-16 sm:py-20 lg:py-24">
      <div className="absolute inset-x-0 top-0 h-[34rem] bg-[radial-gradient(circle_at_12%_12%,rgba(124,58,237,0.14),transparent_32%),radial-gradient(circle_at_88%_48%,rgba(109,40,217,0.1),transparent_30%)]" />
      <Container className="relative z-10">
        ...
        <Card
          className="mt-6 rounded-[2rem] border-[#EDE9FE] bg-white shadow-[0_24px_80px_rgba(124,58,237,0.1)]"
          padding="lg"
          variant="elevated"
        >
          <div className="hidden rounded-2xl border border-[#E9E2F3] bg-[#07030D] px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-white/75 lg:grid lg:grid-cols-[72px_minmax(0,1fr)_104px_112px_92px_92px_minmax(112px,auto)]">
            <span>Rank</span>
            <span>Provider</span>
