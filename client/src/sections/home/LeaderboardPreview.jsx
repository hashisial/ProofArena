...
function LeaderboardRow({ index, reduceMotion, row }) {
  ...
  return (
    <MotionDiv {...getMotionProps(reduceMotion, index * 0.04)}>
      <div className="premium-motion-row grid gap-4 rounded-2xl border border-[#E7E5E4] bg-[#FAFAF9] p-4 transition duration-300 hover:-translate-y-0.5 hover:border-[#65A30D] hover:bg-white hover:shadow-[0_18px_54px_rgba(63, 98, 18, 0.1)] xl:grid-cols-[auto_minmax(0,1fr)_110px_100px_90px] xl:items-center">
        <div className="flex items-center gap-3">
          <span
            className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl text-sm font-black ${
              index === 0 ? "bg-[#3F6212] text-white" : "bg-white text-[#365314]"
            }`}
          >
            {index + 1}
          </span>
          <div className="min-w-0 xl:hidden">
            <p className="font-black leading-6 text-[#1C1917]">{row.provider}</p>
            <p className="text-sm leading-6 text-[#78716C]">{row.category}</p>
          </div>
        </div>

        <div className="hidden min-w-0 xl:block">
          <p className="truncate font-black leading-6 text-[#1C1917]">{row.provider}</p>
          <p className="text-sm leading-6 text-[#78716C]">{row.category}</p>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between gap-3 xl:block">
            ...
            <p className="text-lg font-black text-[#1C1917] xl:mt-1">{row.proofScore}</p>
          </div>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-[#ECFCCB]">
            <div
        ...
        <div className="flex items-center justify-between gap-3 xl:block">
          ...
          <p className="text-base font-black text-[#1C1917] xl:mt-1">
...
export function LeaderboardPreview() {
  ...
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(180deg,#F7FEE7_0%,#F7FEE7_48%,#FFFFFF_100%)] py-16 sm:py-20 lg:py-24">
      <div className="absolute inset-x-0 top-0 h-96 bg-[radial-gradient(circle_at_86%_16%,rgba(63, 98, 18, 0.18),transparent_34%),radial-gradient(circle_at_12%_76%,rgba(10,10,10,0.05),transparent_30%)]" />
      <Container>
          ...
          <MotionDiv {...getMotionProps(reduceMotion, 0)}>
            ...
            <div className="mt-8 grid gap-3">
              {rankingBadges.map((item) => {
                ...
                return (
                  <div className="premium-motion-row flex items-center gap-3 rounded-2xl border border-[#E7E5E4] bg-white px-4 py-3" key={item.label}>
            ...
            <Card className="premium-tilt premium-motion-card overflow-hidden xl:overflow-visible" padding="lg" variant="elevated">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-black text-[#1C1917]">Provider leaderboard</p>
              ...
              <div className="mt-6 hidden rounded-2xl border border-[#E7E5E4] bg-[#1C1917] px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-white/68 xl:grid xl:grid-cols-[auto_minmax(0,1fr)_110px_100px_90px]">
                <span>Rank</span>
                <span>Provider</span>
