...
const statusToneClasses = {
  active: "border-[#A78BFA]/25 bg-[#F5F3FF] text-[#7C3AED]",
  complete: "border-[#7C3AED]/20 bg-[#EDE9FE] text-[#7C3AED]",
  pending: "border-[#6D28D9]/25 bg-[#FEF3C7] text-[#6D28D9]",
  verified: "border-[#A78BFA]/25 bg-[#F5F3FF] text-[#7C3AED]",
};
...
function WorkflowPreview() {
  return (
    <Card
      className="premium-motion-card relative overflow-hidden rounded-[2rem] border-[#EDE9FE] bg-white shadow-[0_24px_80px_rgba(124,58,237,0.12)]"
      padding="lg"
      variant="elevated"
    >
      <div className="absolute inset-x-0 top-0 h-1 bg-[linear-gradient(90deg,#7C3AED,#A78BFA,#6D28D9)]" />
      ...
      <div className="mt-6 rounded-3xl border border-[#E9E2F3] bg-[#F8F4FF] p-4">
      ...
      <div className="mt-5 grid gap-3">
        {previewRows.map((row) => {
          ...
          return (
            <div
              className="premium-motion-row flex items-center gap-3 rounded-2xl border border-[#E9E2F3] bg-white p-3"
              key={row.label}
            >
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-[#F5F3FF] text-[#5B21B6]">
                <Icon aria-hidden="true" className="h-5 w-5" />
              </span>
              <span className="min-w-0 flex-1 text-sm font-black leading-6 text-[#07030D]">
                {row.label}
              </span>
              <span
...
function WorkflowStepCard({ index, isLast, reduceMotion, step }) {
  ...
  return (
    ...
    >
      <div
        aria-hidden="true"
        className="absolute left-6 top-12 h-[calc(100%+1.25rem)] w-px bg-[#EDE9FE] md:hidden"
        hidden={isLast}
      />
      <div className="grid min-w-0 grid-cols-[3rem_minmax(0,1fr)] gap-4 md:block">
        <div className="relative z-10 grid h-12 w-12 place-items-center rounded-2xl border border-[#7C3AED]/20 bg-[#EDE9FE] text-sm font-black text-[#7C3AED] shadow-[0_14px_38px_rgba(124,58,237,0.12)] md:mx-auto md:mb-4">
          {index + 1}
        </div>

        <Card
          className="premium-motion-card h-full rounded-[1.6rem] transition duration-300 hover:-translate-y-1 hover:border-[#A78BFA] hover:shadow-[0_24px_76px_rgba(124,58,237,0.14)]"
          padding="lg"
          variant="default"
        >
          <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <h3 className="mt-6 text-2xl font-black leading-tight text-[#07030D]">
            {step.title}
          </h3>
          ...
          <div className="mt-6 grid gap-2">
            {step.details.map((detail) => (
              <div
                className="flex items-center gap-2 rounded-2xl border border-[#E9E2F3] bg-[#F8F4FF] px-3 py-2"
                key={detail}
              >
                <CheckCircle2 aria-hidden="true" className="h-4 w-4 shrink-0 text-[#7C3AED]" />
                <span className="min-w-0 text-sm font-bold text-[#493C5E]">{detail}</span>
              </div>
            ))}
          </div>
...
export function HomeHowItWorksSection() {
  ...
  return (
    <section className="relative overflow-hidden bg-[linear-gradient(180deg,#FBF9FF_0%,#FFFFFF_46%,#F8F4FF_100%)] py-16 sm:py-20 lg:py-24">
      <div className="absolute inset-x-0 top-0 h-[34rem] bg-[radial-gradient(circle_at_14%_10%,rgba(124,58,237,0.14),transparent_32%),radial-gradient(circle_at_88%_58%,rgba(109,40,217,0.1),transparent_26%)]" />
      <Container className="relative z-10">
        ...
        <div className="relative mt-12">
          <div
            aria-hidden="true"
            className="absolute left-[12.5%] right-[12.5%] top-6 hidden h-px bg-[linear-gradient(90deg,#7C3AED,#A78BFA,#6D28D9)] xl:block"
          />
        ...
        <MotionDiv
          className="mt-8 flex items-start gap-3 rounded-3xl border border-[#E9E2F3] bg-white p-5 shadow-[0_16px_50px_rgba(31,14,54,0.04)]"
          {...getMotionProps(reduceMotion, 0.28)}
        >
          <CalendarCheck aria-hidden="true" className="mt-1 h-5 w-5 shrink-0 text-[#7C3AED]" />
