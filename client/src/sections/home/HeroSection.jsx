...
function ChallengeDetail({ detail }) {
  ...
  return (
    <div className="premium-motion-row rounded-2xl border border-[#E7E5E4] bg-white p-4">
...
function WorkflowRow({ item }) {
  ...
  return (
    <div className="premium-motion-row group rounded-2xl border border-[#E7E5E4] bg-white p-4 transition duration-300 hover:-translate-y-0.5 hover:border-[#65A30D] hover:shadow-[0_18px_42px_rgba(63, 98, 18, 0.12)]">
      <div className="flex items-start gap-3">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-[#F7FEE7] text-[#365314] transition group-hover:bg-[#3F6212] group-hover:text-white">
          <Icon aria-hidden="true" className="h-5 w-5" />
        </span>
        <div className="min-w-0">
          <p className="text-sm font-black text-[#1C1917]">{item.label}</p>
          <p className="mt-1 text-xs leading-5 text-[#78716C]">{item.text}</p>
        </div>
      </div>
    </div>
  );
      ...
      <Container className="grid gap-10 py-14 sm:gap-12 sm:py-20 lg:min-h-[720px] lg:grid-cols-[minmax(0,0.96fr)_minmax(0,1.04fr)] lg:items-center lg:gap-14 lg:py-24 xl:gap-18">
        <div className="min-w-0">
          ...
          <MotionDiv {...getMotionProps(reduceMotion, 0.06)}>
            <h1 className="mt-6 max-w-4xl break-words text-[2.35rem] font-black leading-[1.03] text-[#1C1917] [text-wrap:balance] min-[390px]:text-5xl sm:text-6xl lg:text-7xl">
          ...
          >
            ...
            <a
              className="inline-flex min-h-12 w-full max-w-full items-center justify-center rounded-full px-4 text-center text-sm font-black text-[#365314] transition hover:bg-white/70 focus:outline-none focus:ring-2 focus:ring-[#65A30D]/70 sm:w-auto sm:justify-start"
          ...
          <MotionDiv {...getMotionProps(reduceMotion, 0.24)}>
            <p className="mt-6 max-w-xl text-sm font-bold leading-7 text-[#57534E]">
              Built for clients, providers, and teams who want proof before
              reputation.
            </p>

            <div className="mt-6 rounded-3xl border border-[#E7E5E4] bg-white/80 p-4 shadow-[0_18px_54px_rgba(28, 25, 23, 0.06)] backdrop-blur">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="flex items-center gap-2 text-sm font-black text-[#1C1917]">
                  <Gauge aria-hidden="true" className="h-5 w-5 text-[#3F6212]" />
                  Designed for:
                </div>
                <div className="flex flex-wrap gap-2">
                  {designedFor.map((item) => (
                    <span className="rounded-full border border-[#E7E5E4] bg-[#FAFAF9] px-3 py-2 text-xs font-bold text-[#44403C]" key={item}>
                      {item}
                    </span>
                  ))}
                </div>
        ...
        >
          <div className="relative mx-auto w-full max-w-2xl">
            <div
              aria-hidden="true"
              className="absolute -inset-5 -z-10 rounded-[2.5rem] bg-[#3F6212]/10 blur-2xl"
            />
            ...
            <Card className="premium-tilt premium-motion-card relative overflow-hidden rounded-[2rem] sm:overflow-visible" padding="none" variant="elevated">
              <BackgroundVideo
                ...
                fallbackClassName="bg-[radial-gradient(circle_at_20%_10%,rgba(63, 98, 18, 0.2),transparent_32%),radial-gradient(circle_at_92%_20%,rgba(101, 163, 13, 0.2),transparent_26%),linear-gradient(135deg,rgba(255,255,255,0.96),rgba(243,232,255,0.82))]"
                ...
                <div className="rounded-[1.5rem] border border-[#ECFCCB] bg-[#FEFCE8] p-4 sm:p-5">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <p className="mt-5 text-sm font-black uppercase tracking-[0.16em] text-[#3F6212]">
                        Outcome Challenge
                      </p>
                      <h2 className="mt-2 max-w-lg text-2xl font-black leading-tight text-[#1C1917] sm:text-3xl">
                  ...
                  <div className="mt-5 grid gap-3 rounded-3xl border border-[#E7E5E4] bg-white p-4 sm:grid-cols-3">
                    {previewStats.map((stat) => (
                      <div className="min-w-0 rounded-2xl bg-[#FAFAF9] px-4 py-4" key={stat.label}>
                        <p className="text-xs font-bold uppercase tracking-[0.12em] text-[#78716C]">
                          {stat.label}
                        </p>
                        <p className="mt-2 break-words text-xl font-black text-[#1C1917]">
                          {stat.value}
                        </p>
                      </div>
                    ))}
              ...
              <MotionDiv
                className="float-soft premium-motion-card mx-4 mb-4 rounded-2xl border border-[#3F6212]/20 bg-white p-4 shadow-[0_24px_68px_rgba(63, 98, 18, 0.18)] sm:absolute sm:-bottom-5 sm:left-auto sm:right-8 sm:mx-0 sm:mb-0 sm:w-80"
                {...getRevealMotionProps(reduceMotion, { delay: 0.3, variant: scaleIn })}
              >
                <div className="flex items-center gap-3">
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#F7FEE7] text-[#365314]">
                    <UsersRound aria-hidden="true" className="h-5 w-5" />
                  </span>
                  <div className="min-w-0">
                    <p className="text-sm font-black text-[#1C1917]">12 provider plans submitted</p>
              ...
              <div className="absolute right-3 top-10 hidden rounded-2xl border border-[#E7E5E4] bg-white px-4 py-3 shadow-[0_18px_50px_rgba(28, 25, 23, 0.1)] lg:-right-3 lg:block xl:-right-6">
                <div className="flex items-center gap-2">
                  <CheckCircle2 aria-hidden="true" className="h-5 w-5 text-[#3F6212]" />
                  <span className="text-sm font-black text-[#1C1917]">Proof score 92</span>
                </div>
              </div>
            </Card>
          </div>
