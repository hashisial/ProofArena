...
function CategoryPill({ category, index }) {
  return (
    <a
      className={`group rounded-[1.4rem] border p-5 transition duration-300 hover:-translate-y-1 hover:border-[#7C3AED]/40 hover:shadow-[0_24px_70px_rgba(124, 58, 237, 0.14)] ${
        index === 0
    ...
    >
      <span
        className={`inline-flex h-11 w-11 items-center justify-center rounded-full text-xs font-black ${
          index === 0 ? "bg-[#7C3AED] text-white" : "bg-[#7C3AED]/10 text-[#5B21B6]"
        }`}
      >
...
function MiniService({ service }) {
  return (
    <a
      className="group grid grid-cols-[74px_minmax(0,1fr)] gap-4 rounded-[1.35rem] border border-black/10 bg-white p-3 transition hover:-translate-y-0.5 hover:border-[#7C3AED]/35 hover:shadow-[0_20px_58px_rgba(124, 58, 237, 0.13)]"
      href={`/marketplace/service/${service.slug || service._id}`}
    >
      <div className="overflow-hidden rounded-[1rem] bg-[#12091F]">
        {service.coverImage ? (
          ...
          <div className="h-full min-h-[74px] bg-[linear-gradient(135deg,#07030D,#5B21B6)]" />
        )}
      </div>
      <div className="min-w-0 py-1">
        <p className="line-clamp-2 text-sm font-black leading-5 text-black">{service.title}</p>
        <p className="mt-2 text-xs font-black uppercase tracking-[0.14em] text-[#7C3AED]">
...
function MiniProvider({ provider }) {
  ...
  return (
    <a
      className="rounded-[1.35rem] border border-white/12 bg-white/8 p-4 transition hover:border-[#A78BFA]/42 hover:bg-white/12"
    ...
    >
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
        <motion.div
          ...
          className="absolute right-[8%] top-20 h-64 w-64 rounded-[5rem] border border-[#7C3AED]/12 bg-[#7C3AED]/7"
          transition={{ duration: 32, ease: "linear", repeat: Infinity }}
        />
        <div className="absolute left-[-10rem] top-40 h-[26rem] w-[26rem] rounded-full bg-[#A78BFA]/16 blur-3xl" />
      </div>
      ...
      <div className="relative mx-auto grid w-full max-w-7xl gap-10 px-5 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(18rem,0.95fr)] xl:grid-cols-[minmax(0,1.05fr)_minmax(340px,0.95fr)]">
          ...
          <div className="grid gap-5">
            <div className="rounded-[1.5rem] border border-black/10 bg-[#F8F4FF] p-4 shadow-[0_22px_70px_rgba(124, 58, 237, 0.08)] sm:rounded-[2rem] sm:p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <a className="text-sm font-black text-[#5B21B6] underline decoration-[#A78BFA]/40 underline-offset-4" href="/marketplace">
                  View all
                </a>
              </div>
              <div className="mt-5 grid gap-3">
                {services.length ? services.map((service) => (
                  ...
                  <p className="rounded-[1.25rem] border border-dashed border-[#7C3AED]/22 bg-white p-4 text-sm font-bold text-black/54">
